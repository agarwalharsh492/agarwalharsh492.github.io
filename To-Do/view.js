(function(app){
	...
	View.prototype.render=function(cmd,parameter,isReturning){
		...
		switch(cmd){
			...
			case "updateCount":
				var c=$('.active').length;
				$('#n_tasks').remove();
				$elements.tasksleft.append('<span id="n_tasks">' + c + ((c==1) ? ' task' : ' tasks') + ' left</span>');
				break;
	...
	View.prototype.addTemplateToList = function(val){
		...
		var $elements=this.elem,id=values.id;
		var state=values.status;
		$elements.content.prepend('<div id='+id+' class="'+values.status+' tasks"><input type="checkbox" value="none" class="checkbox mark_com"/><p></p><button type="button" class="delete"><strong>X</strong></button></div>');
		(state==="completed")?state=true:state=false;
		$('#content div:first').find('p').append(values.task);
	...
	View.prototype.deleteTask=function(target){
		var $task=$(target).closest('.tasks'),id=$task.attr('id');
		$task.remove();
		return id;
	};
	...
	View.prototype.showSavedTasks=function(savedTasks,handler){
		var $elements = this.elem,self = this;
